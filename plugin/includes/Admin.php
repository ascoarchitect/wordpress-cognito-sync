...
class Admin {
    ...
    private function toggle_group_sync($group_id, $enabled) {
        ...
        if ($enabled) {
            if (!in_array($group_name, $synced_groups)) {
                ...
                // Create group in Cognito if it doesn't exist
        ...
        $synced_groups = array_values(array_filter($synced_groups)); // Clean up the array
    ...
    public function handle_test_group_sync() {
        ...
        foreach ($synced_groups as $group_name) {
            ...
            $stats['groups'][] = array(
                'name' => "WP_{$group_name}",
                'action' => 'create',  // This would need to be updated to check if group exists in Cognito
    ...
    public function handle_full_sync() {
        ...
        $offset = 0;
        $limit = 20; // Smaller batch size for actual sync
    ...
    private function render_sync_tab() {
        // Clear transients if sync is complete
        if (isset($_GET['sync-complete']) || isset($_GET['test-complete'])) {
            delete_transient('cognito_sync_results');
            delete_transient('cognito_sync_test_results');
            delete_transient('cognito_sync_progress');
        }
    
        if (isset($_GET['group-sync-complete']) || isset($_GET['group-test-complete'])) {
            delete_transient('cognito_sync_group_results');
            delete_transient('cognito_sync_group_test_results');
        }
    
        // Get transients after potential clearing
        ...
        // Add a timestamp to the results if they exist
        if ($test_results) {
            $test_results['timestamp'] = current_time('mysql');
        }
        if ($sync_results) {
            $sync_results['timestamp'] = current_time('mysql');
        }
        if ($group_test_results) {
            $group_test_results['timestamp'] = current_time('mysql');
        }
        if ($group_sync_results) {
            $group_sync_results['timestamp'] = current_time('mysql');
        }
